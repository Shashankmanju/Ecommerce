const data = {
  ...
  function showProducts(category) {
      ...
      selectedCategory.category_products.forEach(product => {
          const productCard = `
              <div class="product-card">
                  ...
                  <h3>${product.title} • <span style="color: #454545; font-size: smaller;">${product.vendor}</span></h3>
  
  
  
                  <p class="price-compare">Rs ${product.price}.00 <span class="compare-price">Rs ${product.compare_at_price}.00</span> <span style="color: red;">50% Off</span></p>
      ...
      // Reset the active class on all tabs
      const tabs = document.querySelectorAll('.tab');
      tabs.forEach(tab => tab.classList.remove('active'));
  
      // Add active class to the clicked tab
      const clickedTab = document.querySelector(`.tab[data-category="${category}"]`);
      clickedTab.classList.add('active');
  
      // Make only the clicked tab black
      tabs.forEach(tab => {
          if (tab !== clickedTab) {
